...
Documented single-backed exception (like hls4ml): `keras.mixed_precision
.Policy` is a per-layer dtype-policy OBJECT with no pytorch_hexagdly
equivalent -- PyTorch's mixed precision (autocast / .half()) works via a
context manager around the forward call, an entirely different mechanism
testing a different thing (no per-layer variable-dtype-vs-compute-dtype
policy object to assert on).
"""
...
def test_conv2d_per_layer_float16_policy(share_neighbors):
    ...
    assert (
        keras.backend.standardize_dtype(weights.dtype) == "float32"
    )  # variables stay float32
