...
port faithful to the original -- a DIFFERENT oracle than test_cross_framework
.py (which compares against this repo's own pytorch_hexagdly fork).

Requires `keras`, `keras_hexagdly`, `torch`, and `hexagdly` (the upstream
package, NOT pytorch_hexagdly) to be installed. Tests are skipped if any is
missing.
...
here -- see test_layers.py and test_depth_padding_keras.py instead.
"""
...
def test_gradients_2d():
    ...
    if keras.backend.backend() != "tensorflow":
        pytest.skip(
            "uses tf.GradientTape directly; only meaningful on the tensorflow backend"
        )
    ...
    for kernel_size in (1, 2, 3):
        ...
        kvars = [getattr(kl, f"kernel{i}") for i in range(kernel_size + 1)] + [
            kl.bias_tensor
        ]
...
def test_share_neighbors_forward_compat_with_upstream():
    ...
    _ = kl(keras.ops.zeros((1, H, W, Cin)))
    rw = (
        tl.ring_weights.detach().numpy()
    )  # assumed (out,in,nr), our own torch-side convention
