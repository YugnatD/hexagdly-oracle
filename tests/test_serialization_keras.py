...
Documented single-backed exception (like hls4ml): get_config/from_config and
the .keras save/load format are a Keras architectural mechanism with no
pytorch_hexagdly equivalent to test -- pytorch_hexagdly is a plain
nn.Module, with no config-dict concept and no .keras-style file format. A
"pytorch version" of this file would only exercise torch's own generic
pickle/state_dict machinery, not any hexagdly-specific code, so there is
nothing meaningful to share here.
"""
