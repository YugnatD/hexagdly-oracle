...
class TestMaxPool2dGather:
    ...
    def test_all_strategies_agree(self):
        ...
        model = _build_2d_model(layer)
        x = RNG.standard_normal((2, H, W, CIN)).astype(np.float32)
        y_slotwise = patch_model_for_hls(model, strategy="slotwise").predict(
            x, verbose=0
        )
...
class TestPatchedModelSerialization:
    ...
    def test_conv2d_maxpool_gather_roundtrip(self, tmp_path, share, bias):
        inp = keras.Input((H, W, CIN))
        x = hgly.Conv2d(
            CIN, COUT, kernel_size=1, bias=bias, share_neighbors=share, name="c1"
        )(inp)
...
class TestConv3dGather:
    ...
    def test_depth_stride_gt1_raises(self):
        ...
        layer = hgly.Conv3d(
            CIN, COUT, kernel_size=(2, 1), stride=(2, 1), depth_padding="valid"
        )
        model = _build_3d_model(layer)
    ...
    def test_parameter_count_reduced(self):
        ...
        assert n_gat < n_slot, (
            f"gather ({n_gat}) should have fewer params than slotwise ({n_slot})"
        )
...
class TestHls4mlCsim:
    ...
    def test_conv2d_gather_ring_sharing_csim(self, tmp_path):
        ...
        layer_share = hgly.Conv2d(
            CIN, COUT, kernel_size=1, bias=False, share_neighbors=True
        )
    ...
    def test_conv2d_gather_stride2_csim(self, tmp_path):
        ...
        layer = hgly.Conv2d(
            1, 2, kernel_size=1, stride=2, bias=False, share_neighbors=True
        )
        inp = keras.Input(shape=(H, W, 1))
        out = layer(inp)
        _rand_weights(layer)
    ...
    def test_conv2d_gather_multichannel_csim(self, tmp_path):
        ...
        Cin_mc, Cout_mc = 3, 4
        layer = hgly.Conv2d(
            Cin_mc, Cout_mc, kernel_size=1, bias=False, share_neighbors=True
        )
    ...
    def test_conv2d_gather_two_hex_layers_csim(self, tmp_path):
        ...
        layer1 = hgly.Conv2d(
            1, 2, kernel_size=1, bias=False, share_neighbors=True, name="hex1"
        )
        layer2 = hgly.Conv2d(
            2, 2, kernel_size=1, bias=False, share_neighbors=True, name="hex2"
        )
        inp = keras.Input(shape=(H, W, 1))
        x1 = layer1(inp)
    ...
    def test_conv3d_gather_csim(self, tmp_path, share, depth_padding):
        ...
        x = RNG.standard_normal((1, D, H, W, CIN)).astype(np.float32)
        y_ref = (
            patch_model_for_hls(model, strategy="slotwise")
            .predict(x, verbose=0)
            .reshape(-1)
        )
    ...
    def test_ring_mac_parallelization_factor(self, tmp_path, pf):
        """ParallelizationFactor must reach the HexRingMAC config as
        ...
        assert mac_names, "no HexRingMAC layer found in config"
    ...
    def test_gather_indices_are_compile_time_constants(self, tmp_path):
        """The gather index table must be emitted as a compile-time constant
        array in the generated config (parameters.h) — not as a runtime weight
        ROM.  With the table constant, the fully-unrolled gather folds to pure
        wiring (the hls4ml im2col-codegen trick), and the plain-int indices can
        never overflow for any camera size.  Likewise the ring map must be
        baked as the constant row_map — no idx/ridx weight files."""
        import glob
        import re
        ...
        global _HLS_DIR
        _HLS_DIR = str(tmp_path / "hls_const_indices")
        ...
        # Constant index table and ring row map are defined out-of-line in
        # parameters.h (same pattern hls4ml uses for conv `pixels[]`).
        assert re.search(r"const int config\d+::indices\[\] = \{", params_txt), (
            "gather indices not emitted as a compile-time constant table"
        )
        assert re.search(r"const unsigned config\d+::row_map\[\] = \{", params_txt), (
            "ring row_map not emitted as a compile-time constant table"
        )
        # No runtime index/ring weight ROMs may be generated.
        weight_files = [
            Path(p).name
            for p in glob.glob(f"{_HLS_DIR}/**/weights/*.h", recursive=True)
        ]
        offenders = [f for f in weight_files if f.startswith(("idx", "ridx"))]
        assert not offenders, f"runtime index weight ROMs still generated: {offenders}"


# =============================================================================
# Linebuffer strategy tests
# =============================================================================
