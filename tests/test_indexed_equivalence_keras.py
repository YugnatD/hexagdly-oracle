...
(neighbor-table builder feeding patch_model_for_hls) -- pytorch-hexagdly has
no hls4ml support at all yet, so there is no `indexed`-equivalent module to
test on that side.
"""
...
# ---- Parity offset tables (line-buffer strategy) ----------------------------


@pytest.mark.parametrize("kernel_size", [1, 2, 3])
@pytest.mark.parametrize("stride", [1, 2, 3])
def test_parity_offset_tables_consistent_with_neighbor_idx(kernel_size, stride):
    """The per-parity (dr, dc) offset tables must reproduce every interior
    neighbor of the flat table: nbr == base + offset, base = stride*(oh, ow)."""
    ...
    K = len(cells)
    assert off_even.shape == (K, 2)
    assert off_odd.shape == (K, 2)

    for n in range(H_out * W_out):
        oh, ow = divmod(n, W_out)
        base_h, base_w = stride * oh, stride * ow
        tab = off_odd if (ow % 2) else off_even
        for k in range(K):
            m = int(nbr[n, k])
            if m < 0:
                continue
            assert (base_h + tab[k, 0]) * Wb + (base_w + tab[k, 1]) == m, (
                f"offset mismatch at pixel {n} slot {k}"
            )
